...
def unload_pipelines():
    if _pipelines:
        logger.info("Unloading pipelines — freeing RAM...")
        _pipelines.clear()
        gc.collect()
        logger.info("✅ RAM freed — idle mode")


async def unload_watcher():
    """Background task: unload model after UNLOAD_AFTER seconds of inactivity."""
    while True:
...
def _synthesize(text, voice, speed, lang_code, split_pattern):
    ...
    chunks = []
    for item in pipeline(text, **kwargs):
        audio = item[-1]  # last element is always audio
        chunks.append(audio)
    if not chunks:
...
async def tts(req: TTSRequest):
    ...
    if voice not in ALL_VOICES:
        raise HTTPException(400, f"Unknown voice '{voice}'. See GET /voices")
    if lang_code not in LANG_MAP:
        raise HTTPException(400, f"Unknown lang_code '{lang_code}'. See GET /languages")
    ...
    try:
        url = await run_in_threadpool(_upload, filename, wav)
    except Exception as e:
        logger.exception("Upload failed")
        raise HTTPException(500, f"Storage error: {e}")

    duration = round(len(audio) / SAMPLE_RATE, 2)
    logger.info(f"✅ {filename} | {duration}s | {voice} | {lang_code}")

    # Update last request time after completion too
    _last_request = time.time()

    return TTSResponse(url=url, filename=filename,
